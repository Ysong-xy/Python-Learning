...
\subsection{uv}
uv类似于pipenv，也是一个python的包管理工具，相较于conda和pipenv，它创建的虚拟环境非常快，因为它使用符号链接而不是复制文件。
uv使用Rust编写，占用的资源更少，但是功能比较有限，主要管理Python包，当设计使用CUDA Toolkit和FFmpeg等系统级工具时，用conda更合适。
\begin{codeblock}[language=bash, caption={uv}]
  uv venv --python=3.10 #创建虚拟环境，会在当前目录下建立.venv目录
  rm -rf .venv #删除环境(直接删除目录即可)
  source .venv/bin/activate #激活虚拟环境
  deactivate #退出虚拟环境
  uv python install 3.12 #下载特定版本python
  uv pip install torch vllm librosa #安装包
  uv pip list #查看已安装包
  uv pip show torch #查看已安装包的详细信息
  uv pip freeze > requirements.txt #导出当前环境到文件
  uv pip install -r requirements.txt #依靠requirements.txt安装环境
  uv sync #依靠pyproject.toml安装环境
\end{codeblock}
...
\begin{codeblock}[language=bash, caption={conda}]
...
\subsection{tmux}
tmux分离的会话和窗口，使得程序运行会话挂起时可以在后台运行而不占用窗口。
\begin{codeblock}[language=bash, caption={tmux command}]
  tmux new -s <session-name> #创建新会话
  tmux detach #分离会话，退出Tmux窗口，会话和进程在后台运行
  tmux ls #查看当前所有的Tmux会话
  tmux attach -t <session-name> #接入已存在的会话
  tmux kill-session -t <session-name> #结束某个会话
  tmux switch -t <session-name> #切换会话
  tmux rename-session -t <old-name> <new-name> #重命名会话
  tmux split-window #上下划分窗口(可选-h左右划分)
\end{codeblock}
